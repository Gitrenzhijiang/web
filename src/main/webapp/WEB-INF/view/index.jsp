								...
								<c:when test="${not empty mainList }">
									<c:forEach items="${mainList}" var="item" varStatus="vs">
										<tr class="success">	
											...
											<td>
												<span><a href="<%=basePath %>questionnaire?method=toeditQuestionnairePage&id=${item.id}" target="_blank">${item.title}</a></span>
											</td>
											<td>
												<span><fmt:formatDate pattern="yyyy-MM-dd" value="${item.ctime }"/></span>
											</td>
											<td>
													...
													<div class="input-group-btn" id="ssdiv">
													   <!-- shiro标签, 不注释会导致页面显示异常 -->
														<!-- <shiro:hasPermission name="wjdctoupiao"> -->
															<a class="btn btn-info" href="<%=basePath %>question?method=joinPartDo&id=${item.id}" target="_blank" >参与投票</a>
														<!-- </shiro:hasPermission> -->
														<!-- <shiro:hasPermission name="wjdcchakan"> -->
															<a class="btn btn-success" href="<%=basePath %>question?method=joinPartDo&id=${item.id}" target="_blank" >查看结果</a>
														<!-- </shiro:hasPermission> -->
													</div>
											</td>
										</tr>	
