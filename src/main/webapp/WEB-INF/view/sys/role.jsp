    ...
    var setting = {
			...
			callback: {
				beforeClick: function(treeId, treeNode) {
					...
					$("#testIframe").attr("src","<%=basePath%>role/toRolePermission?roleId="+treeNode.id);
					return true;
				},
    	...
    	function addTreeNode(){
    		if (zTree.getSelectedNodes()[0]) {
				...
				$("#testIframe").attr("src","<%=basePath%>role/toRoleAdd");
			} else {
    	...
    	function editTreeNode(){
    		if (zTree.getSelectedNodes()[0]) {
				var node = zTree.getSelectedNodes()[0];
				$("#testIframe").attr("src","<%=basePath%>role/toRoleEdit?roleId="+node.id);
			} else {
				//zTree.addNodes(null, newNode);
			}
    	}
    	...
    	function removeTreeNode(){
    		...
    		$.ajax({
    			url:"<%=basePath%>role/roleDele?roleId="+node.id,
    			type:'post',
    			success:function (data){
    				console.info(data);
    				alert ("删除成功");
					window.location="toRole";
    			}
    		}); 
    	}
