	...
	<div class="container">
		...
		<shiro:hasPermission name="wjdcxinjian">
			<div class="collapse" id="collapseNewQuestion">
				<div class="well">
					<form action="<%=basePath %>questionnaire?method=addQuestionnaire" class="form-inline" method="post" id="newQuestion">
						<div class="form-group">
							<label for="mainTitle">问卷标题:</label>
							<input type="text" class="form-control" id="title" name="title" placeholder="关于某某什么的问卷调查">
						</div>
						<div class="form-group date" id="datetimepicker">
							<label for="mainEndtime">截止日期:</label>
							<input type="text" class="form-control" id="mainEndtime" name="dtime" readonly="readonly">
						</div>
						<script type="text/javascript">
							jeDate({
				...
				<c:when test="${not empty mainList }">
					<c:forEach items="${mainList }" var="item" varStatus="vs">
						<tr class="success">	
							...
							<td>
								<span><fmt:formatDate pattern="yyyy-MM-dd hh:mm:ss" value="${item.ctime }"/></span>
							</td>
							<td>
								<span><fmt:formatDate pattern="yyyy-MM-dd" value="${item.dtime }"/></span>
							</td>
							<td>
								<span>
									<c:if test="${item.publish eq '1' }">
										已发布
									</c:if>
									<c:if test="${item.publish eq '0' }">
										未发布
									</c:if>
								</span>
							</td>
							...
							<td class="text-center">
								<div class="input-group-btn" id="ssdiv">
									<shiro:hasPermission name="wjdcshanchu">
										<button class="btn btn-primary" type="button" onclick="delModal('${item.id }')">删除</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="wjdcbianji">
										<a href="<%=basePath %>questionnaire?method=toeditQuestionnairePage&id=${item.id}" class="btn btn-info">编辑</a>
									</shiro:hasPermission>
									<shiro:hasPermission name="wjdcfabu">
										<button type="button" class="btn btn-warning" onclick="actionModel('${item.id }')">发布</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="wjdcfabu">
										<button type="button" class="btn btn-warning" onclick="pauseModel('${item.id }')">停止</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="wjdcfuzhi">
										<button type="button" class="btn btn-danger" onclick="copyQuestion('${item.id }')">复制</button>
									</shiro:hasPermission>
									<shiro:hasPermission name="wjdcchakan">
										<a class="btn btn-success" href="<%=basePath %>question/statistics/${item.id }" target="_blank" >查看结果</a>
									</shiro:hasPermission>
								</div>
							</td>
						</tr>	
